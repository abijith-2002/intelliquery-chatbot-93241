...
class ChatResponse(BaseModel):
    ...
    rag_answer: str = Field(..., description="Preview of retrieved context from uploaded files (used for the answer).")
...
# --- CONTEXT-BASED RETRIEVAL FROM UPLOADED FILES ---
...
def _split_into_chunks(text: str, chunk_size: int = 800, overlap: int = 200) -> List[str]:
    ...
    chunks: List[str] = []
    start = 0
    # Approximate by word-count chunking (assume ~5 chars/word average for simplicity).
...
def _retrieve_relevant_context_from_session(session_id: str, query: str, max_chunks: int = 3) -> Tuple[str, List[Tuple[float, str]]]:
    """
    Retrieve the most relevant context chunks from the uploaded content for this session.

    Returns:
        (joined_context, scored_chunks)
        - joined_context: A concatenation of top chunks (may be empty if no context uploaded).
        - scored_chunks: List of tuples (score, chunk) sorted by score desc.
    """
    session_ctx = CONTEXT_STORE.get(session_id, {})
    combined = session_ctx.get("combined", "") if isinstance(session_ctx, dict) else ""
    if not combined.strip():
        return "", []
    chunks = _split_into_chunks(combined, chunk_size=180, overlap=40)
    if not chunks:
        return "", []
    scored = [(_simple_similarity(query, c), c) for c in chunks]
    scored.sort(key=lambda x: x[0], reverse=True)
    # Use a weak threshold to avoid passing unrelated content
    WEAK_THRESHOLD = 0.10
    filtered = [(s, c) for s, c in scored if s >= WEAK_THRESHOLD][:max_chunks]
    context_to_use = "\n---\n".join([c for _, c in filtered]).strip()
    return context_to_use, filtered
...
def get_gemini_response(
    ...
    Args:
        ...
        rag_answer (str): Legacy parameter (not used for answers.txt anymore). Keep empty or provide brief context, if desired.
    ...
    # Compose prompt with uploaded context primarily; legacy rag_answer is de-emphasized.
    prompt = (
...
@app.post(
    ...
    description="Submit a chat query. Gemini always answers; uploaded context files for the session are included as additional Gemini context if relevant; conversation history is always provided to Gemini for context."
)
def chat(request: ChatRequest):
    """
    PUBLIC_INTERFACE
    Handles user's chat request. All answers come directly from Gemini.
    If the user has uploaded files for this session, the most relevant snippets from those files are provided
    as additional context to Gemini. If no files were uploaded, Gemini answers using only the conversation memory.
    """
    import traceback
    ...
    try:
        ...
        # Context retrieval from uploaded files (replaces answers.txt logic)
        retrieved_context, scored_chunks = _retrieve_relevant_context_from_session(session_id, request.query, max_chunks=3)
        ...
        # Compose Gemini answer with uploaded context (if any)
        try:
        ...
        # For backward compatibility, return retrieved_context in rag_answer field as a preview of what context was used
        # If no files uploaded or nothing relevant found, this may be empty; Gemini still provides an answer based on chat memory.
        preview_context = retrieved_context.strip()
        if len(preview_context) > 1200:
            preview_context = preview_context[:1197] + "..."

        return ChatResponse(
            rag_answer=_safe_str_output(preview_context),  # now represents retrieved context preview
            gemini_answer=_safe_str_output(gemini_answer),  # Gemini's answer, cleaned of meta/source
