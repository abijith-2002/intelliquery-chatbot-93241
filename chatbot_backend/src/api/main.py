...
@app.post(
    ...
    description=(
        "Given a session_id with an uploaded Excel file, this endpoint extracts a compact schema and prompts Gemini "
        "to produce a single-line pandas expression referencing DataFrame 'df'. The expression is evaluated in a "
        "restricted sandbox environment. Returns both the computed JSON result and a narrative generated by Gemini. "
        "No raw Excel data is sent to Gemini; only schema/metadata is provided."
    ),
    responses={
...
def excel_query(request: ExcelQueryRequest):
    ...
    Steps:
        ...
        2) Build a strict prompt for Gemini to output only a pandas expression on 'df'.
        3) Generate expression via Gemini.
        4) Safely evaluate expression with a restricted eval context.
        5) Generate a concise narrative explanation via Gemini.
        6) Return expression, computed result (JSON), and narrative.

    Args:
        request (ExcelQueryRequest): Session id, optional sheet_name, and user query.

    Returns:
        ExcelQueryResponse: The pandas expression, computed result, and a narrative explanation.
    ...
    # Generate a narrative explanation via Gemini (no data, just schema + expression + user request)
    try:
        narration_prompt = (
            "You are given:\n"
